...
func (g *Generator) setdefualtsFromFlags(f flag.Flags) error {
	...
	if f.InterfaceName == "" {
		return fmt.Errorf("specifying interface name is required currently")
	}
...
func (g Generator) writeWrappedClientset() error {
	outIntfContent := new(bytes.Buffer)
	wrapedInf, err := NewWrappedInterface(g.interfaceName, g.inputDir, g.GroupVersions, &codeWriter{out: outIntfContent})
	if err != nil {
		return err
	}

	if err := wrapedInf.writeMethods(); err != nil {
		return err
	}

	outBytes := outIntfContent.Bytes()
	...
	err = g.writeOutCommon(outBytes, "clientset.go")
	if err != nil {
		return err
	}
	return nil
...
func (g Generator) generateHelper(ctx *genall.GenerationContext) error {
	for _, gv := range g.GroupVersions {
		...
		for _, root := range pkgs {
			...
			filename := gv.Group.PackageName() + string(version.Version) + ".go"
			err = g.writeOutCommon(outBytes, filename)
			if err != nil {
				return err
			}
		}
...
func (g *Generator) writeOutCommon(outbytes []byte, filename string) error {
...
func (g *Generator) writeOut(outbytes []byte, filename string) error {
	path := filepath.Join(g.outputDir, "generated", filename)
	if _, err := os.Stat(path); os.IsNotExist(err) {
		err = os.MkdirAll(path, os.ModePerm)
		if err != nil {
			return err
